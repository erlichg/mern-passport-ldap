...
test('renders properly', t => {
  const wrapper = shallowWithIntl(<PostCreateWidget {...props} />);
  ...
  t.truthy(
    wrapper
      .find('h2')
      .first()
      .containsMatchingElement(<FormattedMessage id="createNewPost" />),
  );
...
test('hide when showAddPost is false', t => {
  const wrapper = mountWithIntl(<PostCreateWidget {...props} />);
